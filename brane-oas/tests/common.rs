...
#[allow(dead_code)]
pub fn build_oas_function_body(path: &str, operation_id: &str) -> Result<FunctionAndTypes> {
    build_oas_function(path, operation_id, "body.yml")
}

#[allow(dead_code)]
pub fn build_oas_function_param(path: &str, operation_id: &str) -> Result<FunctionAndTypes> {
    build_oas_function(path, operation_id, "param.yml")
}

#[allow(dead_code)]
pub fn build_oas_function_resp(path: &str, operation_id: &str) -> Result<FunctionAndTypes> {
...
fn build_oas_function(
    ...
    let (functions, types) = build::build_oas_function(operation_id.to_string(), &path_item.get.unwrap(), &oas.components)?;
